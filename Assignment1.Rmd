...
Citizenship Status (CIT) (categorical)
...
Also for the person data, I added PUMA as a variable and filtered this new variable by equating it to Dallas county. Once I ran the R chunk again, the number of observations was reduced to 523, effectively narrowing down the dataset.
...
person_data <- get_pums(variables = c("PUMA",
  ...
  filter(PUMA == dallas_county,
         ...
         WKHP > 0,
         POVPIP > 0) %>%
  select(RAC1P_label, SEX_label, SOCP_label, CIT_label, ENG_label, LANX_label, PINCP, WKHP, POVPIP)
```
